...
//
// whatweb invoked with: whatweb --log-json=/tmp/whatweb.json target.com
// JSON is an array of target objects each with "plugins" map
    ...
    fn parse(&self, output: &str) -> Vec<RawFinding> {
        ...
        for entry in &entries {
            ...
            for (name, data) in plugins {
                ...
                // Check for known vulnerable tech
                if let Some((sev, cves, remediation)) = known_vuln_tech(name, version) {
                    findings.push(RawFinding {
                        ...
                        description: format!(
                            "{} version {} detected on {}. This version has known security vulnerabilities.",
                            name, version, target
                        ),
                    ...
                    // INFO finding for tech detection
                    findings.push(RawFinding {
                        source_tool: "whatweb".to_string(),
                        ...
                        remediation: "Suppress version disclosure headers (X-Powered-By, Server, X-Generator).".to_string(),
                        http_request: None,
                        http_response: None,
                    });
                }
...
fn known_vuln_tech(name: &str, version: &str) -> Option<(&'static str, Vec<String>, &'static str)> {
    ...
    match name_lower.as_str() {
        ...
        "drupal" if ver_major < 9 => Some((
            "HIGH",
            vec!["CVE-2018-7600".to_string()],
            "Update Drupal to 9.x or 10.x. Apply security advisories immediately.",
        )),
        "joomla" if ver_major < 4 => Some((
            "MEDIUM",
            vec![],
        ...
        "php" if ver_major < 8 => Some((
            "HIGH",
            vec!["CVE-2022-31625".to_string(), "CVE-2022-31626".to_string()],
            "Upgrade PHP to 8.1+ immediately. PHP 7.x and earlier are end-of-life.",
        )),
        "jquery" if ver_major == 1 || (ver_major == 2 && ver_minor < 3) => Some((
            "MEDIUM",
            vec!["CVE-2020-11022".to_string(), "CVE-2020-11023".to_string()],
            "Upgrade jQuery to 3.7+. Avoid innerHTML-based DOM manipulation.",
        )),
        "apache" if ver_major == 2 && ver_minor == 4 && version.contains("49") => Some((
            "CRITICAL",
            vec!["CVE-2021-41773".to_string(), "CVE-2021-42013".to_string()],
            "Update Apache immediately. 2.4.49 and 2.4.50 have path traversal RCE.",
        )),
        _ => None,
    }
}
