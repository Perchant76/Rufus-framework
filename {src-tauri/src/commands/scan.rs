...
pub async fn stop_scan(state: State<'_, Mutex<AppState>>) -> Result<(), String> {
    // Drop the lock BEFORE any await point to satisfy Send bound
    let (pool, scan_id) = {
...
pub async fn start_scan(
    ...
    macro_rules! check_running {
        () => {
            if !state.lock().unwrap().scan_running {
                ...
                db::scans::update_status(&pool, &scan_id, "stopped").await.ok();
                return Ok(scan_id);
            }
        };
    }
        ...
        for (host, ip) in crate::parsers::subfinder::parse_assets(&output) {
            ...
            if in_scope {
                let raw = RawFinding {
                    ...
                    description: format!("Subdomain {} found via CT logs / passive OSINT", host),
    ...
    if is_domain && config.tools.contains(&"katana".to_string()) {
        ...
        for endpoint in crate::parsers::katana::parse_endpoints(&output) {
            ...
            let _ = db::assets::insert(&pool, &scan_id, "endpoint", &endpoint,
                None, None, None, None, None, None, in_scope).await;
        }
        for raw in KatanaParser.parse(&output) {
    ...
    if is_domain && config.tools.contains(&"feroxbuster".to_string()) {
        ...
        let output = runner.run("feroxbuster",
            &["-u", &url, "--json", "-t", &threads, "-w", wordlist,
              "-x", "php,asp,aspx,jsp,html,txt,bak,config", "--no-state", "-q", "--silent"],
            None).await.unwrap_or_default();

        for raw in FeroxbusterParser.parse(&output) {
            let in_scope = scope.is_in_scope(&raw.affected_url);
            if let Ok(f) = db::findings::insert(&pool, &scan_id, &raw, in_scope).await {
                runner.emit_finding(&f);
            }
    ...
    if config.tools.contains(&"nuclei".to_string()) {
        ...
        let concurrency = config.concurrency.to_string();
        let mut args = vec!["-u", &url, "-json", "-silent", "-c", &concurrency, "-s", "critical,high,medium,low"];
        ...
        for raw in NucleiParser.parse(&output) {
            let in_scope = scope.is_in_scope(&raw.affected_url);
            if let Ok(f) = db::findings::insert(&pool, &scan_id, &raw, in_scope).await {
                runner.emit_finding(&f);
            }
    ...
    if is_domain && config.tools.contains(&"wapiti3".to_string()) {
        ...
        let content = tokio::fs::read_to_string(&output_file).await.unwrap_or(output);
        for raw in WapitiParser.parse(&content) {
            let in_scope = scope.is_in_scope(&raw.affected_url);
            if let Ok(f) = db::findings::insert(&pool, &scan_id, &raw, in_scope).await {
                runner.emit_finding(&f);
            }
    ...
    if is_domain && config.tools.contains(&"testssl.sh".to_string()) {
        check_running!();
        runner.emit_progress("testssl.sh", 0.0, "[testssl.sh] Analysing TLS...", "info");
    ...
    if config.tools.contains(&"whatweb".to_string()) {
        check_running!();
        runner.emit_progress("whatweb", 0.0, "[whatweb] Fingerprinting tech stack...", "info");
        let url = format!("https://{}", target);
        let output_file = format!("/tmp/whatweb_{}.json", scan_id);
        let _ = runner.run("whatweb",
            &[&url, &format!("--log-json={}", output_file), "-q"],
            None).await;

        let content = tokio::fs::read_to_string(&output_file).await.unwrap_or_default();
        for raw in WhatwebParser.parse(&content) {
            let in_scope = scope.is_in_scope(&raw.affected_url);
            if let Ok(f) = db::findings::insert(&pool, &scan_id, &raw, in_scope).await {
                runner.emit_finding(&f);
            }
    ...
    db::scans::update_status(&pool, &scan_id, "complete").await.map_err(|e| e.to_string())?;
